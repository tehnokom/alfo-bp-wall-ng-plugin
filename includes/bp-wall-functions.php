...
/**
 * Output the 'checked' value, if needed, for a given group comment security on the group create/admin screens
 *
 * @since 0.9.8.6
 *
 ...
 */
function bp_wall_group_show_comment_security_setting( $setting, $group = false ) {
	$group_id = isset( $group->id ) ? $group->id : false;

	$comment_security = bp_wall_group_get_comment_security( $group_id );

	if (is_array($comment_security) && in_array($setting,$comment_security) ) {
...
/**
 * Get the security setting for comments of a group.
 *
 ...
 *                     'mods', and 'admins'.
 */
function bp_wall_group_get_comment_security( $group_id = false ) {
	...
	$comment_security = groups_get_groupmeta( $group_id, 'comment_security' );
	// Backward compatibility. When 'comment_security' is not set, fall back to a default value.
	if ( !$comment_security ) {
		$comment_security = apply_filters( 'bp_wall_group_comment_security_fallback', 'admins' );
	}
	 ...
	 */
	return apply_filters( 'bp_wall_group_get_comment_security', $comment_security, $group_id );
}
 ...
 * @return bool
 */
function bp_wall_groups_user_can_comment( $group_id = 0, $user_id = 0 ) {
	$can_comment = false;
	$comment_security    = false;
	...
	if ( $user_id ) {
			...
			if (is_array($comment_security)) {
				foreach ( $comment_security as $sec ) { 
					switch ( $sec ) {
					case 'admins' :
	...
	/**
	 * Filters whether a user can comment to a group.
	 *
	 * @since 0.9.8.2
	 *
	 * @param bool $can_comment         Whether the user can comment to group wall
	 * @param int  $group_id         The group ID being checked
	 * @param bool $comment_security    The group's current comment security
	 * @param int  $user_id          The user ID being checked
	 */
	return apply_filters( 'bp_wall_groups_user_can_comment', $can_comment, $group_id, $comment_security, $user_id );
}
...
/**
 * Edit the security for a group comments.
 *
 * @since 0.9.8.6
 *
 * @param int    $group_id       ID of the group.
 ...
 * @return bool True on success, false on failure.
 */
function bp_wall_groups_edit_group_comment_security( $group_id, $comment_security ) {

	if ( empty( $comment_security ) ) {
		groups_delete_groupmeta($group_id, 'comment_security');
		return true;
	}
	
	if (!groups_get_groupmeta( $group_id, 'comment_security' )) {
		return groups_add_groupmeta( $group_id, 'comment_security', $comment_security, false);
	} else {
		if (groups_delete_groupmeta($group_id, 'comment_security')) {
			return groups_add_groupmeta( $group_id, 'comment_security', $comment_security, false);
		} else {
			return false;
		}
	 ...
	 * @param int             $group_id          ID of the group.
	 */
	do_action( 'bp_wall_groups_comment_security_updated', $group_id );
...
function bp_wall_member_show_wall_comment_security_setting( $setting ) {
	...
	if (is_array($wall_comment_security) && in_array($setting,$wall_comment_security) ) {
		echo ' checked="checked"';
	}
}
...
/**
 * Get the security setting of a members wall for comments.
 *
 * This function can be used either in or out of the loop.
 *
 * @since 0.9.8.6
...
function bp_wall_member_get_wall_comment_security( $user_id = false ) {
	...
	$wall_comment_security = get_user_meta( $user_id, 'comment_security', true );

	// Backward compatibility. When 'post_security' is not set, fall back to a default value.
	if ( !$wall_comment_security ) {
		$wall_comment_security = apply_filters( 'bp_wall_member_comment_security_fallback', array('') );
	}
	 ...
	 * @param int    $user_id      ID of the user whose status is being checked.
	 */
	return apply_filters( 'bp_wall_member_get_wall_comment_security', $wall_comment_security, $user_id );
}


/**
 * Can a user post a comment to a other_user wall?
 *
 * @since 0.9.8.6
 *
 * @param int $user_id  The user ID to check.
 ...
 * @return bool
 */
function bp_wall_user_can_comment( $user_id = 0, $other_user_id = 0 ) {
	global $bp,$bp_wall;
	$can_post = false;
	$wall_security    = false;
	...
	if ( $other_user_id ) {
		if ($user_id == $other_user_id) {
			...
			$wall_security = bp_wall_member_get_wall_comment_security( $other_user_id );
	 ...
	 */
	return apply_filters( 'bp_wall_user_can_comment', $can_post, $user_id, $wall_security, $other_user_id );
}
...
function bp_wall_user_can_post( $user_id = 0, $other_user_id = 0 ) {
	 ...
	 * @param int  $other_user_id          The other_user ID being checked
	 */
	return apply_filters( 'bp_wall_user_can_post', $can_post, $user_id, $wall_security, $other_user_id );
}
...
/**
 * Edit the security for a wall comments.
 *
 * @since 0.9.8.6
 *
 * @param int    $user_id       ID of the user.
 ...
 * @return bool True on success, false on failure.
 */
function bp_wall_member_edit_comment_security( $user_id, $comment_security ) {
	#error_log("SECCC".print_r($comment_security,1));
	if ( empty( $comment_security ) ) {
		delete_user_meta($user_id, 'comment_security');
		return true;
	}
	
	if (!get_user_meta( $user_id, 'comment_security', true )) {
		#error_log("SETTT");
		return add_user_meta( $user_id, 'comment_security', $comment_security, false);
	} else {
		#error_log("NOTTSETTT");
		if (delete_user_meta($user_id, 'comment_security')) {
			#error_log("DELETEADD");
			#error_log(print_r(add_user_meta( $user_id, 'comment_security', $comment_security, false),1));
			#error_log(print_r("USERMETA".get_user_meta( $user_id, 'comment_security', true )),1);
			#return true;
			return add_user_meta( $user_id, 'comment_security', $comment_security, false);
		} else {
			return false;
	 ...
	 * @param int             $user_id          ID of the user.
	 */
	do_action( 'bp_wall_comment_security_updated', $user_id );
